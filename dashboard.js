...
const signedInUser = (() => { try { return JSON.parse(localStorage.getItem("aiGymCurrentUser") || "null"); } catch { return null; } })();
const storageKey = `aiGymAppState:${signedInUser?.tenantId || "default"}`;
  ...
  if (role === "admin") {
    return `
      ...
      <table class="data-table">
        ...
        <tbody>${adminRows || '<tr><td colspan="4">No member subscriptions yet.</td></tr>'}</tbody>
      </table>
    `;
  }

  const userMember = selectedMember(state);
  if (!userMember) return `<div class="subscription-status"><strong>No membership</strong><span>Ask your gym administrator to create your member profile.</span></div>`;
...
function invoiceReportHtml(state, invoice, logoDataUrl = "") {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escapeAttribute(invoice.number)}</title><style>body{font:14px Arial;color:#2f2925;margin:42px}header{display:flex;justify-content:space-between;border-bottom:3px solid #ff7417;padding-bottom:22px}.brand{font-size:28px;font-weight:800;color:#ff7417}.invoice-brand{display:flex;flex-direction:column;align-items:flex-start}.invoice-logo{display:block;width:230px;height:86px;object-fit:contain;object-position:left center;margin:0 0 5px}.invoice-title{text-align:right}h1{margin:0;font-size:26px}.meta{display:grid;grid-template-columns:1fr 1fr;gap:28px;margin:28px 0}.box{padding:18px;border:1px solid #ddd;border-radius:12px}.box b{display:block;margin-bottom:8px}table{width:100%;border-collapse:collapse}th,td{padding:12px;border-bottom:1px solid #ddd;text-align:left}th{background:#fff2e8}.totals{width:340px;margin:24px 0 24px auto}.totals div{display:flex;justify-content:space-between;padding:8px}.totals .grand{font-size:18px;font-weight:800;border-top:2px solid #ff7417}.status{display:inline-block;padding:7px 12px;border-radius:20px;background:#fff2e8;color:#d85d0e;font-weight:700}.notes{margin-top:18px;padding:18px;background:#f7f6f4;border-radius:12px}.notes p{white-space:pre-wrap}footer{margin-top:40px;border-top:1px solid #ddd;padding-top:16px;color:#777}@media(max-width:650px){body{margin:20px}.invoice-logo{width:180px;height:68px}.meta{grid-template-columns:1fr}.totals{width:100%}}@media print{body{margin:20px}}</style></head><body><header><div class="invoice-brand">${brand}<div>Fitness Management Portal</div></div><div class="invoice-title"><h1>DETAILED TAX INVOICE</h1><p>${escapeAttribute(invoice.number)}</p><span class="status">${escapeAttribute(invoice.paymentStatus)}</span></div></header><section class="meta"><div class="box"><b>Bill To</b>${escapeAttribute(recipient.name)}<br>${escapeAttribute(recipient.email || `${invoice.recipientType} ${invoice.recipientId}`)}<br>Recipient type: ${escapeAttribute(invoice.recipientType)}<br>Recipient ID: ${escapeAttribute(invoice.recipientId)}</div><div class="box"><b>Invoice Details</b>Issue date: ${escapeAttribute(invoice.issueDate)}<br>Due date: ${escapeAttribute(invoice.dueDate)}<br>Billing period: ${escapeAttribute(invoice.billingPeriod || "—")}<br>Reference / PO: ${escapeAttribute(invoice.purchaseOrder || "—")}<br>Issued by: Gym Admin</div></section><table><thead><tr><th>#</th><th>Description</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead><tbody>${rows}</tbody></table><section class="totals"><div><span>Subtotal</span><b>${money(totals.subtotal)}</b></div><div><span>Discount</span><b>- ${money(totals.discount)}</b></div><div><span>GST / tax (${Number(invoice.taxRate || 0)}%)</span><b>${money(totals.tax)}</b></div><div class="grand"><span>Grand total</span><b>${money(totals.total)}</b></div></section><section class="notes"><b>Notes</b><p>${escapeAttribute(invoice.notes || "—")}</p></section><section class="notes"><b>Payment terms</b><p>${escapeAttribute(invoice.terms || "Payment is due by the stated due date.")}</p></section><footer>This invoice was generated by the gym administrator on ${escapeAttribute(invoice.issuedAt ? new Date(invoice.issuedAt).toLocaleString("en-IN") : invoice.createdAt ? new Date(invoice.createdAt).toLocaleString("en-IN") : invoice.issueDate)}.</footer></body></html>`;
}
...
function renderAdminInvoicesPanel(state) {
  ...
  const emptyNotice = recipients ? "" : `<div class="invoice-empty"><strong>Add a member or trainer first</strong><span>Invoices can only be addressed to members and trainers in this gym workspace.</span></div>`;
  return `<article class="card admin-insight-card invoice-admin-card"><h2>Detailed Invoice Generation</h2><p>Admin invoices can only be issued to this gym's trainers and members. Create a detailed draft, verify it, then issue it to the recipient dashboard.</p>${emptyNotice}<form class="dashboard-form invoice-form" data-action="createInvoice"><label>Recipient type and account<select name="recipient" required ${recipients ? "" : "disabled"}>${recipients || '<option value="">No recipients available</option>'}</select></label><label>Billing period<input name="billingPeriod" placeholder="July 2026 / Personal training package"></label><label>Issue date<input type="date" name="issueDate" value="${today}" required></label><label>Due date<input type="date" name="dueDate" value="${due.toISOString().slice(0, 10)}" required></label><label>Invoice reference / PO<input name="purchaseOrder" placeholder="Optional internal reference"></label><label>Payment status<select name="paymentStatus"><option>Unpaid</option><option>Paid</option><option>Partial</option></select></label><label class="invoice-wide">Detailed line items <small>One per line: Description | Quantity | Rate</small><textarea name="items" rows="6" placeholder="Monthly membership | 1 | 1999&#10;Personal training sessions | 8 | 500&#10;Nutrition consultation | 1 | 750" required></textarea></label><label>Discount (INR)<input type="number" name="discount" min="0" step="0.01" value="0"></label><label>GST / tax rate (%)<input type="number" name="taxRate" min="0" max="100" step="0.01" value="18"></label><label class="invoice-wide">Notes<textarea name="notes" rows="3" placeholder="Service details, GST information, or a message for the recipient"></textarea></label><label class="invoice-wide">Payment terms<textarea name="terms" rows="3">Payment is due by the stated due date. Please mention the invoice number in the payment reference.</textarea></label><button type="submit" ${recipients ? "" : "disabled"}>Create detailed draft invoice</button></form>${renderInvoiceCards(state, [...state.invoices].reverse(), true)}</article>`;
}

function renderPlatformInvoicesPanel() {
  if (!platformInvoices.length) return `<article class="card admin-insight-card"><h2>AI Gym SaaS Invoices</h2><p>No platform invoices have been issued to this gym.</p></article>`;
  return `<article class="card admin-insight-card invoice-admin-card"><h2>AI Gym SaaS Invoices</h2><p>Invoices issued by the platform owner for your gym workspace.</p><div class="invoice-list">${platformInvoices.map(invoice=>`<article class="invoice-card"><div><span>${escapeAttribute(invoice.number)}</span><strong>${escapeAttribute(invoice.billingPeriod||"Platform services")}</strong><small>Issued ${escapeAttribute(invoice.issueDate)} · Due ${escapeAttribute(invoice.dueDate)}</small></div><div><strong>${money(invoice.total)}</strong><span class="invoice-status ${invoice.paymentStatus==='Paid'?'paid':'unpaid'}">${escapeAttribute(invoice.paymentStatus)}</span><small>${invoice.items.length} detailed line item${invoice.items.length===1?'':'s'}</small></div><div class="invoice-actions"><button type="button" data-action="downloadPlatformInvoice" data-id="${escapeAttribute(invoice.id)}">Download detailed invoice</button></div></article>`).join("")}</div></article>`;
}

function platformInvoiceReport(invoice) {
  const rows=invoice.items.map(item=>`<tr><td>${escapeAttribute(item.description)}</td><td>${item.quantity}</td><td>${money(item.rate)}</td><td>${money(item.quantity*item.rate)}</td></tr>`).join("");
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escapeAttribute(invoice.number)}</title><style>body{font:14px Arial;color:#25211e;max-width:900px;margin:40px auto;padding:30px}header{display:flex;justify-content:space-between;border-bottom:4px solid #ff7417;padding-bottom:22px}h1{color:#ff7417;margin:0}section{display:grid;grid-template-columns:1fr 1fr;gap:30px;margin:28px 0}table{width:100%;border-collapse:collapse}th,td{padding:12px;border-bottom:1px solid #ddd;text-align:left}.totals{margin:25px 0 25px auto;width:360px}.totals div{display:flex;justify-content:space-between;padding:8px}.total{font-size:20px;font-weight:bold;border-top:2px solid #ff7417}footer{margin-top:45px;padding-top:20px;border-top:1px solid #ddd;color:#666;white-space:pre-wrap}</style></head><body><header><div><h1>AI GYM SaaS</h1><p>Platform services invoice</p></div><div><strong>${escapeAttribute(invoice.number)}</strong><p>Status: ${escapeAttribute(invoice.paymentStatus)}</p></div></header><section><div><h3>Billed to</h3><strong>${escapeAttribute(invoice.tenantName)}</strong><p>${escapeAttribute(invoice.billingEmail||"")}</p></div><div><h3>Invoice details</h3><p>Issue: ${invoice.issueDate}<br>Due: ${invoice.dueDate}<br>Period: ${escapeAttribute(invoice.billingPeriod||"—")}<br>Reference: ${escapeAttribute(invoice.purchaseOrder||"—")}</p></div></section><table><thead><tr><th>Description</th><th>Quantity</th><th>Rate</th><th>Amount</th></tr></thead><tbody>${rows}</tbody></table><div class="totals"><div><span>Subtotal</span><strong>${money(invoice.subtotal)}</strong></div><div><span>Discount</span><strong>− ${money(invoice.discount)}</strong></div><div><span>Tax (${invoice.taxRate}%)</span><strong>${money(invoice.tax)}</strong></div><div class="total"><span>Total</span><strong>${money(invoice.total)}</strong></div></div><footer><h3>Notes</h3>${escapeAttribute(invoice.notes||"—")}<h3>Payment terms</h3>${escapeAttribute(invoice.terms||"—")}<p>Created by ${escapeAttribute(invoice.createdBy||"AI Gym SaaS")} on ${escapeAttribute(invoice.createdAt)}</p></footer></body></html>`;
}
...
function renderMemberAttendancePanel(state) {
  ...
  const lastCheckIn = Array.isArray(state.attendanceLog)
    ...
    : null;
  const todayRecord = Array.isArray(state.attendanceLog) ? state.attendanceLog.find((item) => item.memberId === member.id && item.date === todayDateKey()) : null;
    ...
    <div class="member-attendance-card member-attendance-summary-card">
      ...
      <div class="member-attendance-summary-body">
        ...
        <ul class="task-list compact-admin-list member-mini-list">
          ...
          <li><span>Automatic checkout</span><strong>8 hours or 11:59 PM</strong></li>
      ...
      <div class="member-action-grid">
        <button class="wide-action" type="button" data-action="memberCheckIn" ${todayRecord ? "disabled" : ""}>${todayRecord ? "Checked in today" : "Mark attendance now"}</button>
        <button class="wide-action" type="button" data-action="memberCheckOut" ${!todayRecord || todayRecord.checkOutTime ? "disabled" : ""}>${todayRecord?.checkOutTime ? "Checked out" : "Check out"}</button>
      </div>
...
function renderDashboard() {
  ...
  const platformInvoicePanel = dashboardRoot.querySelector("[data-platform-invoices]");
  if (platformInvoicePanel) platformInvoicePanel.innerHTML = role === "admin" ? renderPlatformInvoicesPanel() : "";
...
function bindActions(role, state) {
  ...
  interactionRoot.querySelectorAll("form[data-action]").forEach((form) => {
    form.addEventListener("submit", async (event) => {
      ...
      if (form.dataset.action === "createInvoice") {
        ...
        if (!['member', 'trainer'].includes(recipientType) || !recipientId) { window.alert('Select a valid member or trainer.'); return; }
        const items = String(data.items || "").split(/\r?\n/).map((line) => {
          const [description, quantity, rate] = line.split("|").map((value) => value.trim());
          return { description, quantity: Number(quantity), amount: Number(rate) };
        }).filter((item) => item.description && item.quantity > 0 && item.amount >= 0);
        if (!items.length) { window.alert('Add at least one valid line item using Description | Quantity | Rate.'); return; }
        ...
        state.invoices.push({
          ...
          purchaseOrder: data.purchaseOrder?.trim() || "",
          items,
          ...
          terms: data.terms?.trim() || "Payment is due by the stated due date.",
  ...
  interactionRoot.querySelectorAll("button[data-action]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      if (button.dataset.action === "downloadPlatformInvoice") {
        const invoice = platformInvoices.find((item) => item.id === button.dataset.id);
        if (invoice) downloadTextFile(`${invoice.number}.html`, platformInvoiceReport(invoice), "text/html");
        return;
      }
      ...
      if (button.dataset.action === "memberCheckIn") {
        const member = selectedMember(state);
        if (!state.attendanceLog.some((item) => item.memberId === member.id && item.date === todayDateKey())) {
          const now = new Date();
          const time = new Intl.DateTimeFormat("en-IN", { hour: "2-digit", minute: "2-digit" }).format(now);
          state.attendanceLog.unshift({ id: `A${String(state.attendanceLog.length + 1).padStart(4, "0")}`, memberId: member.id, date: todayDateKey(), time });
          member.attendance = Number(member.attendance || 0) + 1;
          member.progress = Math.min(100, Number(member.progress || 0) + 2);
          addActivity(state, `${member.name} marked attendance`, "Check-in");
        }
      }

      if (button.dataset.action === "memberCheckOut") {
        const member = selectedMember(state);
        const record = state.attendanceLog.find((item) => item.memberId === member.id && item.date === todayDateKey());
        if (record && !record.checkOutTime) record.checkOutTime = new Intl.DateTimeFormat("en-IN", { hour: "2-digit", minute: "2-digit" }).format(new Date());
      }
...
async function hydrateStateFromServer() {
  try {
    const response = await fetch(`${apiBaseUrl}/api/state`, { headers: { "Authorization": `Bearer ${currentUser()?.token || ""}` } });
    if (response.status === 401 || response.status === 403) { localStorage.removeItem("aiGymCurrentUser"); window.location.href = "login.html"; return; }
    if (!response.ok) throw new Error("State API unavailable");
    const state = await response.json();
    if (currentUser()?.role === "admin") {
      const invoiceResponse = await fetch(`${apiBaseUrl}/api/platform-invoices`, { headers: { "Authorization": `Bearer ${currentUser()?.token || ""}` } });
      if (invoiceResponse.ok) platformInvoices = (await invoiceResponse.json()).invoices || [];
    }
    localStorage.setItem(storageKey, JSON.stringify(state));
